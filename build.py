...
def home_page() -> str:
    ...
    return doc(
        ...
        <main>
            ...
            <div class="container hero-grid">
              ...
              <div class="showcase-grid">
                ...
                <article class="card strong">
                  ...
                  <h3>제품 메뉴 안에서 콘텐츠 허브부터 결제와 결과 전달까지 한 번에 이어집니다</h3>
                  <p class="lead" style="font-size:1rem">각 제품 상세는 콘텐츠 허브를 먼저 보고, 제품 설명을 이해하고, 데모 시연 뒤 결제하면 바로 결과 전달까지 이어지는 실행형 페이지입니다.</p>
                  <div class="badge-row"><span class="badge">데모 시연</span><span class="badge">결제</span><span class="badge">결과 전달</span><span class="badge">콘텐츠 허브 함께 보기</span></div>
                </article>
              </div>
            </div>
          </section>
          ...
          <section class="section"><div class="container"><div class="section-head"><div><h2>고객 상황에 맞춰 바로 비교하고 선택할 수 있는 4개 제품</h2></div><p>제품 메뉴를 누르면 각 제품으로 이동해 콘텐츠 허브·제품 설명·데모 시연·결제·결과 전달까지 바로 이어집니다. 제품별 콘텐츠 허브는 상세 상단에서 먼저 확인할 수 있습니다.</p></div><div class="product-grid" id="product-grid"></div></div></section>
        </main>
        '''),
        page_key='home',
        ...
        <section class="section">
          <div class="container page-hero">
            <div class="card strong">
              ...
              <p class="lead">각 제품 상세 안에서 콘텐츠 허브, 제품 설명, 데모 시연, 결제, 결과 전달까지 자연스럽게 이어집니다. 제품별 콘텐츠 허브는 제품 상세 상단에서 먼저 둘러볼 수 있습니다.</p>
            </div>
            <div class="card accent"><span class="tag" style="background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.12);color:#fff">결제 흐름</span><h3 style="font-size:1.72rem;margin:16px 0 10px">콘텐츠 허브 → 제품 설명 → 데모 시연 → 결제 → 결과 전달</h3><p>복잡한 문의 절차보다, 직접 써보고 바로 결정할 수 있는 흐름을 우선합니다.</p></div>
          </div>
        </section>
...
def product_page(product: dict) -> str:
    ...
    return doc(f"{product['name']} | {brand['name']}", product['summary'], product['theme'], dedent(f'''
        <main>
            ...
            <div class="container page-hero">
              <div class="card strong">
                ...
                <div class="inline-tabs">
                  <a href="#board">콘텐츠 허브</a>
                  <a href="#intro">제품 소개</a>
                  <a href="#demo">데모</a>
                  <a href="#order">결제 진행</a>
                  <a href="#payment">결과 전달 안내</a>
                  <a href="#delivery">전달 일정</a>
                </div>
              </div>
          ...
          <section class="section compact" id="board"><div class="container"><div class="section-head"><div><h2>{escape(product['name'])} 콘텐츠 허브</h2></div><p>제품을 자세히 보기 전에 먼저 읽어보면 좋은 핵심 글을 모아두었습니다. 연관 주제 글을 충분히 읽고, 마음이 정리되면 제품 설명·데모 시연·결제로 이어질 수 있습니다.</p></div><div class="board-grid" id="product-board-grid"></div><div id="product-post-detail"></div></div></section>
          ...
          <section class="section compact" id="demo"><div class="container module-layout"><article class="card strong"><span class="tag theme-chip">데모 시연</span><h3>몇 가지 정보만 입력하면 샘플 결과를 바로 보고 데모 시연 자료까지 확인할 수 있습니다</h3><form id="product-demo-form" class="stack-form"><div class="form-grid"><div><label>회사명</label><input name="company" data-demo-field="company" placeholder="샘플 브랜드" autocomplete="organization" required></div><div><label>담당자명</label><input name="name" data-demo-field="name" placeholder="담당자명" autocomplete="name" required></div><div><label>이메일</label><input name="email" type="email" data-demo-field="email" placeholder="email@company.com" autocomplete="email" inputmode="email" required></div><div><label>연락처</label><input name="phone" data-demo-field="phone" placeholder="예: 010-1234-5678" inputmode="tel" autocomplete="tel"></div><div><label>팀 규모</label><input name="team" data-demo-field="team" placeholder="예: 2인 운영팀" autocomplete="organization-title"></div><div><label>목표</label><input name="goal" data-demo-field="goal" placeholder="예: CTA 전환 개선" required></div><div><label>핵심 키워드</label><input name="keywords" data-demo-field="keywords" placeholder="예: 랜딩, CTA, 신뢰"></div><div><label>참고 링크</label><input name="link" data-demo-field="link" placeholder="예: https://example.com" inputmode="url" autocomplete="url"></div><div><label>긴급도</label><select name="urgency" data-demo-field="urgency"><option value="">선택</option><option>일반</option><option>이번 주 안</option><option>오늘 필요</option></select></div><div><label>플랜 미리보기</label><select name="plan" data-demo-field="plan"><option value="Starter">Starter</option><option value="Growth">Growth</option><option value="Scale">Scale</option></select></div></div><div class="actions"><button class="button" type="submit">데모 시연 시작하기</button><a class="button ghost" href="#order">이 조건으로 결제 계속하기</a></div></form><div class="result-box" id="product-demo-result" role="status" aria-live="polite"></div></article><article class="card"><span class="tag">체험 포인트</span><h3>데모에서 먼저 보시면 좋은 항목</h3><ul class="clean" id="product-demo-scenarios"></ul></article></div></section>
          <section class="section compact" id="order"><div class="container module-layout"><article class="card strong"><span class="tag theme-chip">결제</span><h3>플랜을 고른 뒤 1회 결제 정보를 입력하면 결제와 전달 준비를 한 번에 정리합니다</h3><form id="product-checkout-form" class="stack-form"><input type="hidden" name="product" value="{escape(product['key'])}"><div class="form-grid"><div><label>플랜</label><select name="plan" data-prefill="plan" required><option value="Starter">Starter</option><option value="Growth">Growth</option><option value="Scale">Scale</option></select></div><div><label>결제 유형</label><select name="billing"><option value="one-time">1회 결제형</option></select></div><div><label>결제 방식</label><select name="paymentMethod" required><option value="toss">Toss 결제</option></select></div><div><label>회사명</label><input name="company" placeholder="회사명" autocomplete="organization" required></div><div><label>담당자명</label><input name="name" placeholder="담당자명" autocomplete="name" required></div><div><label>이메일</label><input name="email" type="email" placeholder="email@company.com" autocomplete="email" inputmode="email" required></div><div><label>연락처</label><input name="phone" placeholder="예: 010-1234-5678" inputmode="tel" autocomplete="tel"></div><div><label>참고 링크</label><input name="link" placeholder="예: https://example.com" inputmode="url" autocomplete="url"></div><div><label>긴급도</label><select name="urgency"><option value="">선택</option><option>일반</option><option>이번 주 안</option><option>오늘 필요</option></select></div><div><label>추가 요청</label><input name="note" placeholder="예: 원하는 톤, 꼭 포함할 내용" autocomplete="off"></div></div><div class="actions"><button class="button" type="submit">결제 계속하기</button><a class="button secondary" href="#payment">결과 전달 안내 보기</a></div><p class="micro-copy">기업 정산, 환불 기준, 예외 범위는 <a href="{prefix}pricing/index.html">가격</a>, <a href="{prefix}legal/refund/index.html">환불정책</a>, <a href="{prefix}contact/index.html?product={escape(product['key'])}">추가 확인</a>에서 바로 확인할 수 있습니다.</p></form><div class="result-box" id="product-checkout-result" role="status" aria-live="polite"></div></article><article class="card"><span class="tag">플랜</span><h3>지금 바로 선택할 수 있는 플랜</h3><div class="plan-grid" id="plan-grid"></div></article></div></section>
          <section class="section compact" id="payment"><div class="container module-layout"><article class="card strong"><span class="tag theme-chip">자동 제공 안내</span><h3>결제가 완료되면 결과 전달 준비가 자동으로 이어집니다</h3><ol class="flow-list"><li>결제 정보를 먼저 저장합니다.</li><li>Toss 결제를 선택하면 외부 결제창으로 바로 이동합니다.</li><li>결제가 확인되면 결과 자료와 공개 콘텐츠가 즉시 준비됩니다.</li><li>결과 전달 상태가 같은 조회 코드로 묶입니다.</li><li>포털에서 결과 자료와 관련 콘텐츠를 함께 확인합니다.</li></ol><div class="notice">전자동 실행이 기본입니다. 기업 정산이 필요한 경우에도 먼저 자동 흐름과 동일한 결과 기준을 맞춘 뒤 별도 확인으로 이어집니다.</div></article><article class="card strong"><span class="tag theme-chip">가격 기준</span><h3>시장 비교 기준</h3><p id="product-pricing-basis"></p><div class="notice">처음 시작하는 팀도 부담 없이 바로 적용해 볼 수 있는 범위를 기준으로 플랜을 나눴습니다.</div></article></div></section>
          <section class="section compact" id="delivery"><div class="container module-layout"><article class="card strong"><span class="tag theme-chip">전달 일정</span><h3>결제 직후 전달 일정과 공개 콘텐츠 연결이 자연스럽게 이어집니다</h3><ol class="flow-list" id="product-workflow"></ol></article><article class="card strong"><span class="tag theme-chip">연결 확장 서비스</span><h3>이 제품에 바로 붙여 판매할 수 있는 모듈</h3><div class="admin-grid" id="product-service-stats"></div><div class="accordion-stack" id="product-service-catalog"></div><div class="small-actions"><a href="{prefix}service/index.html">전체 확장 서비스 180개 보기</a></div></article></div></section>
          <section class="section compact"><div class="container module-layout"><article class="card strong"><span class="tag theme-chip">함께 보면 좋은 제품</span><h3>비슷한 고민에 이어서 보기 좋은 제품</h3><div class="story-grid" id="product-related-modules"></div></article><article class="card strong"><span class="tag theme-chip">연결 방식</span><h3>핵심 제품 1개 + 필요한 서비스만 추가하는 구조</h3><div class="kv"><div class="row"><strong>기본 원칙</strong><span>반복 작업은 자동화하고 예외 건만 운영자가 확인합니다.</span></div><div class="row"><strong>판매 구조</strong><span>핵심 진단·발행·알림 제품을 먼저 제안한 뒤 필요한 보완만 추가합니다.</span></div><div class="row"><strong>운영 관점</strong><span>1인 운영에서도 유지 가능한 범위와 자동화 우선순위를 같이 보여줍니다.</span></div></div></article></div></section>
                    <section class="section compact"><div class="container"><div class="section-head"><div><h2>자주 묻는 질문</h2></div><p>제품 결제와 결과 전달 전에 자주 나오는 질문을 먼저 정리했습니다.</p></div><div class="faq-grid" id="product-faq"></div></div></section>
        </main>
