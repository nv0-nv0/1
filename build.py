...
def home_page() -> str:
    ...
    return doc(
        ...
        <main>
          ...
          <section class="section compact"><div class="container"><div class="section-head"><div><h2>결제 전에도 결과물 느낌을 먼저 보여주는 이유</h2></div><p>설명만 보고 결정하기보다 실제 전달 화면이 어떤 식으로 정리되는지 먼저 보는 편이 전환과 신뢰 모두에 유리합니다. 그래서 샘플 리포트 느낌, 플랜 비교 기준, 데모 이후 이동 경로를 홈에서 먼저 보여줍니다.</p></div><div class="report-mock-grid"><article class="mock-report-card"><span class="tag">샘플 리포트 01</span><h3>위험 신호 요약 카드</h3><p>가장 먼저 봐야 할 위기 점수, 우선순위, 예상 노출 구간을 짧게 보여줍니다.</p><ul class="clean"><li>위기 점수와 우선 조치</li><li>예상 과태료 노출 구간</li><li>즉시 수정 우선순위</li></ul></article><article class="mock-report-card"><span class="tag">샘플 리포트 02</span><h3>페이지별 수정안 묶음</h3><p>결제 후에는 페이지 단위로 무엇을 바꿔야 하는지, 어떤 문구가 들어가야 하는지 같은 실행 자료로 이어집니다.</p><ul class="clean"><li>페이지별 문제 요약</li><li>교체 문구안과 위치</li><li>재점검 기준</li></ul></article><article class="mock-report-card"><span class="tag">샘플 리포트 03</span><h3>전달·발행·이력 연결</h3><p>결과물을 받은 뒤 끝나는 구조가 아니라, 고객 포털과 자료실 글, 관리자 이력까지 같은 코드로 이어지게 설계했습니다.</p><ul class="clean"><li>조회 코드 기반 포털 확인</li><li>발행 글 연결</li><li>다음 수정 때 재사용</li></ul></article></div></div></section>
          ...
          <section class="section compact admin-shell" id="admin-shell">
            <div class="container admin-layout">
              ...
              <div class="admin-main-stack">
                ...
                <section id="admin-automation" class="admin-section"><div class="record-grid" id="admin-automation-grid"></div><div class="card strong"><h3>자료실 CTA 자동 발행 설정</h3><form id="admin-board-settings-form" class="stack-form"><div class="form-grid"><div><label>기본 CTA 문구</label><input name="ctaLabel" placeholder="예: 제품 설명 보기"></div><div><label>기본 CTA 링크</label><input name="ctaHref" placeholder="예: /products/veridion/index.html#intro"></div><div><label>발행 주기</label><select name="scheduleType"><option value="daily">daily</option><option value="weekly">weekly</option><option value="interval">interval</option></select></div><div><label>회차당 발행 수</label><input name="frequencyPerRun" type="number" min="1" max="24" value="1"></div><div><label>반복 간격(시간)</label><input name="intervalHours" type="number" min="1" max="168" value="24"></div><div><label>발행 시각</label><input name="timeSlots" placeholder="예: 09:00, 15:00"></div><div class="span-2"><label>선택 제품</label><input name="selectedProducts" placeholder="예: veridion, clearport"></div><div><label>발행 모드</label><select name="publishMode"><option value="publish">publish</option><option value="draft">draft</option></select></div></div><label class="consent-check"><input type="checkbox" name="autoPublishEnabled" value="1"> <span>자동 발행 사용</span></label><label class="consent-check"><input type="checkbox" name="autoPublishAllProducts" value="1"> <span>전체 제품에 동일 CTA 자동 발행 적용</span></label><div class="actions"><button class="button" type="submit">설정 저장</button><div class="inline-actions"><select name="publishNowProduct" id="admin-publish-now-product"><option value="">자동 대상 전체</option><option value="veridion">Veridion</option><option value="clearport">ClearPort</option><option value="grantops">GrantOps</option><option value="draftforge">DraftForge</option></select><input name="publishNowCount" id="admin-publish-now-count" type="number" min="1" max="24" value="1" style="max-width:110px"><button class="button ghost" type="button" id="admin-publish-all">즉시 발행</button></div></div></form></div></section>
