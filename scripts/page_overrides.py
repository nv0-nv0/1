    ...
    body = dedent(f'''    <main>
      ...
      <section class="section compact">
        <div class="container">
          ...
          <div class="report-mock-grid" style="margin-top:18px">
            <article class="mock-report-card">
              <span class="tag">샘플 전달 화면</span>
              <h3>핵심 결과 요약</h3>
              <p>처음 받는 화면에서 지금 가장 먼저 손대야 할 항목과 이유가 보이도록 정리합니다.</p>
              <ul class="clean"><li>핵심 요약</li><li>우선 수정 순서</li><li>바로 실행 기준</li></ul>
            </article>
            <article class="mock-report-card">
              <span class="tag">플랜 비교 기준</span>
              <h3>추천 플랜과 시작가를 바로 비교</h3>
              <div class="plan-rail">{''.join(f'<div class="plan-rail-card{' recommended' if plan.get('recommended') else ''}"><strong>{escape(plan['name'])}</strong><span>{escape(plan['price'])}</span><small>{escape(plan.get('note', ''))}</small></div>' for plan in product.get('plans', [])[:3])}</div>
            </article>
            <article class="mock-report-card">
              <span class="tag">데모 다음 단계</span>
              <h3>데모 후 결제로 이어질 때 열리는 것</h3>
              <ul class="clean"><li>세부 결과와 전달 자산</li><li>자료실 연동 글</li><li>고객 포털 조회 코드</li></ul>
              <div class="small-actions"><a href="{prefix}products/{escape(product['key'])}/demo/index.html">즉시 데모</a><a href="{prefix}products/{escape(product['key'])}/plans/index.html">플랜 보기</a></div>
            </article>
          </div>
