...
def legal_privacy_page(brand: dict) -> str:
    ...
    body = dedent(f'''    <main>
      <section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../../index.html">HOME</a><span class="sep">/</span><span>개인정보처리방침</span></div><h1>개인정보처리방침</h1><p class="lead">NV0는 데모, 결제, 예외 문의, 포털 조회에 필요한 최소 정보만 처리합니다. 공개 화면과 관리자 화면을 분리해 고객용 화면에서 운영용 정보가 과하게 드러나지 않도록 관리합니다.</p><div class="notice policy-meta-box"><strong>시행일</strong> {POLICY_EFFECTIVE_DATE}<br><strong>최종 개정일</strong> {POLICY_UPDATED_DATE}<br><strong>문의</strong> <a href="mailto:{email}">{email}</a></div><div class="small-actions"><a href="../terms/index.html">이용약관</a><a href="../refund/index.html">환불정책</a><a href="../cookies/index.html">쿠키 및 저장 안내</a></div></div></div></section>
      <section class="section compact"><div class="container module-layout"><article class="card strong"><span class="tag">처리 목적</span><div class="kv"><div class="row"><strong>문의/데모</strong><span>요청 접수, 샘플 저장, 후속 안내</span></div><div class="row"><strong>결제</strong><span>주문 등록, 결제 확인, 결과 제공</span></div><div class="row"><strong>포털</strong><span>조회 코드 기반 상태 확인</span></div><div class="row"><strong>운영 기록</strong><span>재발행, 상태 변경, 감사 추적</span></div></div></article><article class="card strong"><span class="tag">수집 항목</span><div class="kv"><div class="row"><strong>기본 항목</strong><span>회사명, 담당자명, 이메일, 선택 제품, 요청 내용</span></div><div class="row"><strong>결제 항목</strong><span>플랜, 결제 상태, 조회 코드, 결제 결과 확인 정보</span></div><div class="row"><strong>선택 항목</strong><span>전화번호, 참고 링크, 일정, 추가 요청 메모</span></div><div class="row"><strong>보관 원칙</strong><span>업무 목적 달성 후 지체 없이 파기하고, 법령상 보존 의무가 있을 때만 별도 보관합니다.</span></div></div></article></div></section>
      <section class="section compact"><div class="container module-layout"><article class="card strong"><span class="tag">처리·보관</span><ul class="clean"><li>결제 연동이 활성화된 경우 결제 완료 확인과 정산을 위해 필요한 최소 정보만 결제대행사로 전달될 수 있습니다.</li><li>브라우저 저장소에는 편의 기능과 상태 유지에 필요한 최소 정보만 저장합니다.</li><li>공개 사이트와 관리자 화면은 분리되어 있으며, 관리자 인증 정보는 관리자 화면에서만 사용합니다.</li></ul></article><article class="card strong"><span class="tag">이용자 권리</span><ul class="clean"><li>이용자는 언제든지 안내 이메일로 열람·정정·삭제를 요청할 수 있습니다.</li><li>처리 목적이 끝난 정보는 지체 없이 정리하고, 분쟁·정산 대응이 필요한 경우 법령 범위 안에서만 보관합니다.</li><li>문의 시 본인 확인을 위해 추가 확인을 요청할 수 있습니다.</li></ul></article></div></section>
    </main>
    ''')
...
def legal_refund_page(brand: dict) -> str:
    email = escape(brand.get('contact_email', ''))
    body = dedent(f'''    <main>
      <section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../../index.html">HOME</a><span class="sep">/</span><span>환불정책</span></div><h1>환불정책</h1><p class="lead">업무 착수 전에는 전액 환불을 원칙으로 하고, 맞춤형 산출물 작성이나 외부 비용이 시작된 뒤에는 진행 범위와 제공된 결과물을 기준으로 정산합니다.</p><div class="notice policy-meta-box"><strong>시행일</strong> {POLICY_EFFECTIVE_DATE}<br><strong>최종 개정일</strong> {POLICY_UPDATED_DATE}<br><strong>문의</strong> <a href="mailto:{email}">{email}</a></div><div class="small-actions"><a href="../terms/index.html">이용약관</a><a href="../privacy/index.html">개인정보처리방침</a></div></div></div></section>
      <section class="section compact"><div class="container module-layout"><article class="card strong"><span class="tag">환불 기준</span><div class="kv"><div class="row"><strong>착수 전</strong><span>업무 미착수, 외부 비용 없음, 맞춤 산출물 미제공 시 전액 환불 원칙</span></div><div class="row"><strong>착수 후</strong><span>진행 범위, 투입 시간, 외부 비용, 제공 완료 결과물 기준으로 부분 정산</span></div><div class="row"><strong>결제 수단</strong><span>결제 수단별 승인 취소, 부분 취소, 환급 시점은 결제 사업자 및 카드사 정책에 따라 달라질 수 있습니다.</span></div></div></article><article class="card strong"><span class="tag">처리 순서</span><ol class="flow-list"><li>환불 요청 접수</li><li>진행 상태·외부 비용·제공 결과물 확인</li><li>정산 기준 안내</li><li>승인 취소 또는 환급 처리</li></ol></article></div></section>
      <section class="section compact"><div class="container"><article class="card strong"><span class="tag">예시</span><ul class="clean"><li>결제 직후 바로 취소하고 작업이 시작되지 않은 경우: 전액 환불 원칙</li><li>데모/검토를 마치고 맞춤 산출물이 일부 제공된 경우: 진행분 정산 후 부분 환불 가능</li><li>외부 결제 수수료나 별도 실비가 발생한 경우: 해당 비용은 제외 후 정산</li></ul></article></div></section>
    </main>
    ''')
...
def legal_cookies_page(brand: dict) -> str:
    email = escape(brand.get('contact_email', ''))
    body = dedent(f'''    <main>
      <section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../../index.html">HOME</a><span class="sep">/</span><span>쿠키 및 저장 안내</span></div><h1>쿠키 및 저장 안내</h1><p class="lead">이 사이트는 데모, 문의, 결제 준비, 포털 조회 편의를 위해 브라우저 저장소를 사용할 수 있습니다. 고객용 공개 화면과 관리자용 인증 정보는 저장 목적과 범위를 다르게 관리합니다.</p><div class="notice policy-meta-box"><strong>시행일</strong> {POLICY_EFFECTIVE_DATE}<br><strong>최종 개정일</strong> {POLICY_UPDATED_DATE}<br><strong>문의</strong> <a href="mailto:{email}">{email}</a></div></div></div></section>
      <section class="section compact"><div class="container module-layout"><article class="card strong"><span class="tag">저장 항목</span><div class="kv"><div class="row"><strong>세션 저장소</strong><span>관리자 화면 접속 시 임시 인증 토큰 저장</span></div><div class="row"><strong>로컬 저장소</strong><span>데모, 문의, 주문, 공개 글 상태의 로컬 캐시</span></div><div class="row"><strong>거부 영향</strong><span>브라우저 저장을 막으면 일부 편의 기능과 상태 유지가 제한될 수 있습니다.</span></div></div></article><article class="card strong"><span class="tag">원칙</span><ul class="clean"><li>관리자 인증 정보는 관리자 화면에서만 사용합니다.</li><li>공개 화면에는 운영 기능을 노출하지 않도록 분리합니다.</li><li>브라우저 저장을 지우면 임시 기록과 편의 기능 상태가 함께 초기화됩니다.</li></ul></article></div></section>
    </main>
    ''')
...
def product_demo_page(brand: dict, product: dict) -> str:
    ...
    quick_demo_buttons = ''.join(
        f'<button class="quick-demo-button" type="button" title="{escape(str(item if isinstance(item, str) else str(item)))}" data-quick-demo="{escape(product["key"])}" data-quick-scenario="{idx}">예시 {idx + 1} · {escape(short_text(item if isinstance(item, str) else str(item), 26))}</button>'
        for idx, item in enumerate(product.get('demo_scenarios', [])[:4])
    )
    body = dedent(f'''    <main><section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../../../index.html">HOME</a><span class="sep">/</span><a href="../../index.html">제품</a><span class="sep">/</span><a href="../index.html">{escape(product['name'])}</a><span class="sep">/</span><span>즉시 데모</span></div><span class="tag theme-chip">{escape(product['label'])}</span><h1>{escape(product['name'])} 즉시 데모</h1><p class="lead">미리보기로 방향을 먼저 확인하고, 저장은 원하실 때만 하실 수 있게 했습니다. 결제로 넘어가기 전에 핵심 결과를 먼저 판단하실 수 있습니다.</p>{product_subnav('../../../', product, 'demo')}</div><div class="card theme-panel"><span class="tag" style="background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.12);color:#fff">먼저 볼 예시</span><ul class="clean inverse-list">{sample_points}</ul></div></div></section><section class="section compact"><div class="container module-layout"><article class="card strong"><span class="tag theme-chip">빠른 미리보기</span><div class="quick-demo-grid">{quick_demo_buttons}</div><div class="result-box" id="quick-demo-result" role="status" aria-live="polite"></div></article><article class="card strong"><span class="tag theme-chip">저장하기</span><form id="product-demo-form" class="stack-form"><div class="form-grid"><div><label>회사명</label><input name="company" data-demo-field="company" placeholder="샘플 브랜드" autocomplete="organization" required></div><div><label>담당자명</label><input name="name" data-demo-field="name" placeholder="담당자명" autocomplete="name" required></div><div><label>이메일</label><input name="email" type="email" data-demo-field="email" placeholder="email@company.com" autocomplete="email" inputmode="email" required></div><div><label>팀 규모</label><input name="team" data-demo-field="team" placeholder="예: 2인 운영팀"></div><div><label>목표</label><input name="goal" data-demo-field="goal" placeholder="예: CTA 전환 개선" required></div><div><label>핵심 키워드</label><input name="keywords" data-demo-field="keywords" placeholder="예: 랜딩, CTA, 신뢰"></div><div><label>참고 링크 (선택)</label><input name="link" placeholder="대표 URL 또는 참고 문서"></div><div><label>플랜 미리보기</label><select name="plan" data-demo-field="plan"><option value="Starter">Starter</option><option value="Growth">Growth</option><option value="Scale">Scale</option></select></div><div class="span-2"><label>추가 메모 (선택)</label><textarea name="context" rows="4" placeholder="꼭 확인하고 싶은 장면을 적어 주세요"></textarea></div></div>{consent_notice('../../../', 'product-demo')}<div class="actions"><button class="button" type="submit">샘플 저장하기</button><a class="button ghost" href="../plans/index.html">플랜 보기</a></div></form><div class="result-box" id="product-demo-result" role="status" aria-live="polite"></div></article></div></section></main>
    ''')
