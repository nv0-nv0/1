...
def pricing_page(brand: dict, products: list[dict]) -> str:
    ...
    cards = ''.join(
        f"<article class='story-card {escape(item['theme'])}'><span class='tag theme-chip'>{escape(item['label'])}</span><h3>{escape(item['name'])}</h3><p>{escape(item['pricing_basis'])}</p><div class='small-actions'><a href='../products/{escape(item['key'])}/index.html'>제품 보기</a><a href='../checkout/index.html?product={escape(item['key'])}'>지금 시작하기</a></div></article>"
        for item in products
    )
    body = dedent(f'''
    <main>
      <section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../index.html">HOME</a><span class="sep">/</span><span>가격</span></div><span class="kicker">Pricing</span><h1>가격, 범위, 결과물을 한 번에 비교하고 바로 시작할 수 있습니다</h1><p class="lead">제품 정의 데이터와 같은 원천에서 가격을 불러와 제품 상세, 시작 흐름, 가격 안내가 서로 어긋나지 않도록 맞췄습니다.</p><div class="actions"><a class="button" href="../checkout/index.html">지금 시작하기</a><a class="button secondary" href="../products/index.html">제품 비교</a></div></div><div class="card accent"><span class="tag" style="background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.12);color:#fff">가격 원칙</span><h3 style="font-size:1.72rem;margin:16px 0 10px">가격과 설명이 서로 어긋나지 않게 관리합니다</h3><p>시작가와 플랜 설명이 제품 데이터와 같은 소스에서 나오도록 구성해 가격 불일치를 줄였습니다.</p></div></div></section>
...
def legal_privacy_page(brand: dict) -> str:
    body = dedent(f'''
    <main><section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../../index.html">HOME</a><span class="sep">/</span><span>개인정보처리방침</span></div><h1>개인정보처리방침</h1><p class="lead">문의, 지금 시작하기, 포털 조회에 필요한 최소 범위의 정보를 처리합니다. 실제 운영 환경의 위탁사, 보관 기간, 국외 이전 여부는 연동 서비스가 확정되면 이 문서에 즉시 반영해야 합니다.</p><div class="kv"><div class="row"><strong>문의 이메일</strong><span>{escape(brand.get('contact_email',''))}</span></div><div class="row"><strong>주요 처리 목적</strong><span>문의 응대, 지금 시작하기, 결제/포털 안내, 운영 기록 관리</span></div><div class="row"><strong>기본 보관 원칙</strong><span>업무 목적 달성 후 지체 없이 파기, 법령상 보존 의무 시 별도 분리 보관</span></div></div></div></div></section></main>
    ''')
