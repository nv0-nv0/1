...
def pricing_page(brand: dict, products: list[dict]) -> str:
    ...
    cards = ''.join(
        f"<article class='story-card {escape(item['theme'])}'><span class='tag theme-chip'>{escape(item['label'])}</span><h3>{escape(item['name'])}</h3><p>{escape(item['pricing_basis'])}</p><div class='small-actions'><a href='../products/{escape(item['key'])}/index.html'>제품 설명 보기</a><a href='../products/{escape(item['key'])}/index.html#demo'>데모 시연</a><a href='../checkout/index.html?product={escape(item['key'])}'>결제 진행</a></div></article>"
        for item in products
    )
    body = dedent(f'''
    <main>
      <section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../index.html">HOME</a><span class="sep">/</span><span>가격</span></div><span class="kicker">Pricing</span><h1>가격, 범위, 1회 결제형 자료를 한 번에 비교하고 바로 시작할 수 있습니다</h1><p class="lead">제품 정의 데이터와 같은 원천에서 가격을 불러와 제품 상세, 시작 흐름, 가격 안내가 서로 어긋나지 않도록 맞췄습니다.</p><div class="actions"><a class="button" href="../products/index.html">제품 비교</a><a class="button secondary" href="../demo/index.html">데모 시연</a><a class="button ghost" href="../checkout/index.html">결제 진행</a></div></div><div class="card accent"><span class="tag" style="background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.12);color:#fff">가격 원칙</span><h3 style="font-size:1.72rem;margin:16px 0 10px">가격과 설명이 서로 어긋나지 않게 관리합니다</h3><p>시작가와 플랜 설명이 제품 데이터와 같은 소스에서 나오도록 구성해 가격 불일치를 줄였습니다.</p></div></div></section>
      ...
      <section class="section compact"><div class="container"><div class="section-head"><div><h2>가격 설명</h2></div><p>가격만이 아니라 어떤 결과물을 받는지, 몇 영업일 기준으로 진행되는지 같이 확인할 수 있게 정리했습니다.</p></div><div class="story-grid">{cards}</div></div></section>
    </main>
    ''')
...
def docs_page(brand: dict, products: list[dict]) -> str:
    cards = ''.join(
        f"<article class='story-card {escape(item['theme'])}'><span class='tag theme-chip'>{escape(item['label'])}</span><h3>{escape(item['name'])} 시작 안내</h3><p>{escape(item['summary'])}</p><div class='small-actions'><a href='../docs/{escape(item['key'])}/index.html'>열기</a><a href='../products/{escape(item['key'])}/index.html'>제품 상세</a></div></article>"
        for item in products
    )
...
def legal_privacy_page(brand: dict) -> str:
    body = dedent(f'''
    <main><section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../../index.html">HOME</a><span class="sep">/</span><span>개인정보처리방침</span></div><h1>개인정보처리방침</h1><p class="lead">추가 확인, 데모 시연, 결제 진행, 포털 조회에 필요한 최소 범위의 정보를 처리합니다. 실제 운영 환경의 위탁사, 보관 기간, 국외 이전 여부는 연동 서비스가 확정되면 이 문서에 즉시 반영해야 합니다.</p><div class="kv"><div class="row"><strong>안내 이메일</strong><span>{escape(brand.get('contact_email',''))}</span></div><div class="row"><strong>주요 처리 목적</strong><span>추가 확인 응대, 데모 시연 저장, 결제 진행, 정상작동 및 발행 제공/포털 안내, 운영 기록 관리</span></div><div class="row"><strong>폼 기준 필수 항목</strong><span>추가 확인: 관심 제품·회사명·담당자명·이메일·확인 내용 / 데모·결제 진행: 제품·회사명·담당자명·이메일</span></div><div class="row"><strong>기본 보관 원칙</strong><span>업무 목적 달성 후 지체 없이 파기, 법령상 보존 의무 시 별도 분리 보관</span></div></div></div></div></section></main>
    ''')
...
def legal_refund_page(brand: dict) -> str:
    body = dedent(f'''
    <main><section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../../index.html">HOME</a><span class="sep">/</span><span>환불정책</span></div><h1>환불정책</h1><p class="lead">착수 전에는 전액 환불을 원칙으로 하되, 맞춤형 자료 작성·분석·외부 비용이 이미 시작된 뒤에는 진행 범위와 기 제공 산출물을 기준으로 부분 환불 여부를 판단합니다. 전자동 결제와 기업 정산 추가 확인 모두 동일한 기준을 적용하되, 소비자 법령이 적용되는 경우 해당 법령을 우선합니다.</p><div class="kv"><div class="row"><strong>착수 전</strong><span>업무 미착수·외부 비용 없음·맞춤 산출물 미작성 시 전액 환불 원칙</span></div><div class="row"><strong>착수 후</strong><span>진행 범위, 작성 시간, 기 제공 결과물, 외부 비용 공제 후 정산</span></div><div class="row"><strong>접수 채널</strong><span>{escape(brand.get('contact_email',''))}</span></div></div></div></div></section></main>
    ''')
...
def service_page(brand: dict) -> str:
    body = dedent('''
    <main><section class="section"><div class="container page-hero"><div class="card strong"><div class="crumbs"><a href="../index.html">HOME</a><span class="sep">/</span><span>진행 흐름</span></div><span class="kicker">Service flow</span><h1>결제 후에는 포털에서 정상작동 상태와 발행 제공 자료를 이어서 확인합니다</h1><p class="lead">사이트 소개에서 끝나지 않고, 자동발행게시판 → 설명 → 데모 시연 → 결제 → 정상작동 및 발행 제공 순서로 이어지게 설계했습니다.</p><div class="actions"><a class="button" href="../checkout/index.html">결제 진행</a><a class="button secondary" href="../portal/index.html">고객 포털</a><a class="button ghost" href="../contact/index.html">추가 확인</a></div></div></div></section></main>
    ''')
