(function(){
  ...
  const navItems = [
    ...
    ['왜 NV0인가', `${base}engine/index.html`, 'engine'],
    ['제품', `${base}products/index.html`, 'products'],
    ['문제별 선택', `${base}solutions/index.html`, 'solutions'],
    ['가격', `${base}pricing/index.html`, 'pricing'],
    ['도입 문서', `${base}docs/index.html`, 'docs'],
    ['추가 서비스', `${base}service/index.html`, 'service'],
    ['FAQ', `${base}faq/index.html`, 'faq'],
    ['콘텐츠', `${base}board/index.html`, 'board'],
    ['조건 문의', `${base}contact/index.html`, 'contact'],
  ];
  const STORE = {
  ...
  function renderHeader(){
    ...
    const quickLinks = isProductSurface && productKey
      ? [`<a href="${base}products/${productKey}/index.html" class="sub-head ${pageKey === 'product' ? 'active' : ''}">제품 소개</a>`, `<a href="${base}products/${productKey}/demo/index.html" class="sub-link ${path.includes(`/products/${productKey}/demo/`) ? 'active' : ''}">샘플 결과</a>`, `<a href="${base}products/${productKey}/plans/index.html" class="sub-link ${path.includes(`/products/${productKey}/plans/`) ? 'active' : ''}">플랜</a>`, `<a href="${base}products/${productKey}/delivery/index.html" class="sub-link ${path.includes(`/products/${productKey}/delivery/`) ? 'active' : ''}">전달 자료</a>`, `<a href="${base}products/${productKey}/faq/index.html" class="sub-link ${path.includes(`/products/${productKey}/faq/`) ? 'active' : ''}">FAQ</a>`, `<a href="${base}products/${productKey}/board/index.html" class="sub-link ${path.includes(`/products/${productKey}/board/`) ? 'active' : ''}">콘텐츠</a>`].join('')
      : `<a href="${base}products/index.html" class="sub-head ${pageKey === 'products' ? 'active' : ''}">제품</a>${productLinks}`;
    header.innerHTML = `<button class="admin-fab" type="button" data-admin-entry="1" title="비밀키가 있어야 열립니다">관리자</button><div class="container nav-wrap"><div class="nav-left"><a class="brand" href="${base}index.html"><span class="brand-mark">N0</span><span class="brand-copy"><strong>${config.brand?.name || 'NV0'}</strong><span>${config.brand?.tagline || ''}</span></span></a></div><nav class="nav-links">${navItems.map(([label, href, key]) => `<a href="${href}" class="${pageKey === key ? 'active' : ''}">${label}</a>`).join('')}<a class="button secondary" href="${base}demo/index.html">샘플 결과</a><a class="button" href="${base}pricing/index.html">가격 확인</a></nav></div><div class="container subnav">${quickLinks}</div>`;
  }
  ...
  function buildProductSpecificDemoResult(values){
    ...
    return `
      <div class="demo-result-shell">
        ...
        ${result.extra}
        <div class="demo-save-box"><strong>이번 결과 한 줄 제품 소개</strong><br>${esc(result.note || '')}</div>
      </div>
    `;
  }
